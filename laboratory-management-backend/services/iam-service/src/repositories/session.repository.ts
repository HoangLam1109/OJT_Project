...
export class UserSessionRepository implements IUserSessionRepository {
  constructor(private userSessionModel: any, private redisClient: any) {}
  ...
  async create(sessionData: Partial<IUserSession>): Promise<IUserSession> {
    ...
    if (this.redisClient) {
      ...
      console.log(
        `[SESSION REPO] Redis disabled - skipping cache for new session ${savedSession._id}`
      );
    }
  ...
  async invalidateAllUserSessions(userId: string): Promise<number> {
    ...
    if (this.redisClient) {
      console.log(
        `[SESSION REPO] Redis disabled - skipping cache invalidation for user ${userId}`
      );
    }
  ...
  async findById(id: string, fields?: string): Promise<IUserSession | null> {
    return await this.userSessionModel.findById(id, fields).populate("userId");
  }

  async findByUserId(userId: string): Promise<IUserSession[]> {
    return await this.userSessionModel.find({ userId }).populate("userId");
  }
  ...
  async updateById(
    ...
    return await this.userSessionModel
      .findByIdAndUpdate(id, sessionData, { new: true })
      .populate("userId");
  }
  ...
  async cacheSession(sessionId: string, sessionData: any): Promise<void> {
    if (this.redisClient) {
      ...
      console.log(
        `[SESSION REPO] Redis disabled - skipping cache for session ${sessionId}`
      );
    }
  }
  ...
  async getCachedSession(token: string): Promise<IUserSession | null> {
    if (this.redisClient) {
      ...
      console.log(
        `[SESSION REPO] Redis disabled - cache miss for token ${token}`
      );
    }
    return null;
  }
